...
import {dispatchEmailLogin} from '../context/action/authAction';
...
import {SIGNUP, HOME, FORGOTPW, LOGIN} from '../constants/routers';
...
export default function Loginemail() {
  ...
  const {user_email, user_password, save} = user;
  const isInvalid = user_email === '' || user_password === '' 
  ...
  const handleSignin = async (e) => {
    ...
    await dispatchEmailLogin(dispatch, {user_email, user_password, save})
  }
    ...
    <div className="min-h-screen bg-black font-Roboto select-none flex flex-col">
      ...
      <form className="mx-auto flex mt-[5rem] flex-col">
        ...
        <div className="mt-2 flex text-white justify-between text-sm p-2 ">
          ...
          <Link href={LOGIN}>
            <span className="cursor-pointer hover:text-blue-400">Login With User Id</span>
          </Link>
        </div>
        <button className={`${isInvalid && 'opacity-60'} border-none h-10  font-bold font-xl mt-5
          rounded-md text-black bg-[#0A8F00]`}
          disable={isInvalid || state.isLoading} type="submit" data-testid="sign-in"
